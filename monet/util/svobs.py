...
class SObs(object):
    ...
    def __init__(self, dates, area, states=['nd'], tdir='./'):
        ...
        self.pfile = './' + 'obs' + \
            self.d1.strftime("%Y%m%d.") + self.d2.strftime("%Y%m%d.") + 'pkl'
    ...
    def read_csv(self, name, hdrs=[0]):
        ...
        def to_datetime(d): return datetime.datetime.strptime(
            d, '%Y-%m-%d %H:%M:%S')
        obs = pd.read_csv(name, sep=',', header=hdrs,
    ...
    def find(self, verbose=False, pload=True, getairnow=False, tdir='./',
        ...
        if test:
            ...
            try:
                ...
                pload = False
            if pload:
                met_obs = self.read_csv(
                    tdir + 'met' + self.csvfile, hdrs=[0, 1])
            # except:
        ...
        if not pload:
            ...
            if getairnow:
                ...
                aq = aqs_mod.AQS()
                aq.add_data([self.d1, self.d2], param=[
                            'SO2', 'WIND', 'TEMP', 'RHDP'], download=False)
    ...
    def check(self):
        ...
        sra = self.met[('siteid', 'Unnamed: 2_level_1')].unique()
        print(sra)
        for site in sra:
